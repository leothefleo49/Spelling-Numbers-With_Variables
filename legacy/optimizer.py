"""  
Optimization Solver
Uses genetic algorithm or SciPy differential evolution to find optimal letter values.
"""
...
try:
    ...
    psutil = None
try:
    from scipy import optimize as so  # type: ignore
except ImportError:
    so = None
...
class LetterValueOptimizer:
    ...
    Uses a genetic algorithm approach or SciPy differential evolution.
    """
    ...
    def __init__(self, start_range: int, end_range: int, population_size: int = 100,
                 ...
                 hyphen_operator: str = 'add', cpu_usage: Union[str, int] = 'auto',
        ...
        if cpu_usage == 'auto':
            self.cpu_cores = total_cores  # Use all cores for max speed
        ...
        self.is_solving = False  # For stopping optimization

    def precompute_structures(self):
        """Precompute letter count matrix, usage stats, and parsed components."""
        counts = []
        targets = []
        self.component_cache = []
        self.letter_usage_counts = {letter: 0 for letter in self.letters}
        for num in self.numbers:
            ...
            for letter in self.letters:
                letter_count = flat_spelling.count(letter)
                row.append(letter_count)
                self.letter_usage_counts[letter] += letter_count
            counts.append(row)
            targets.append(num)
        if np is not None:
            try:
        ...
        self.letter_usage_rank = sorted(self.letter_usage_counts.keys(),
                                        key=lambda letter: (self.letter_usage_counts[letter], letter))
        self.rare_letters = self.letter_usage_rank[:8]
    ...
    def analyze_constraints(self):
        ...
        # Fix Z=0 if 0 in range
        if 0 in self.numbers:
            self.letter_constraints['Z'] = 0.0
            logger.debug('Fixed Z=0.0 based on ZERO presence')
        else:
            self.letter_hints['Z'] = (0.0, 0.1)  # Still hint near zero
        ...
        self.frequency_solutions = self.frequency_guided_solver(letter_counts)
        if self.frequency_solutions:
            for letter, value in self.frequency_solutions.items():
                if letter in self.letter_constraints:
                    continue
                span = max(0.1, abs(value) * 0.25 + 0.05)
                lo, hi = value - span, value + span
                if letter in self.letter_hints:
                    existing = self.letter_hints[letter]
                    lo = min(lo, existing[0])
                    hi = max(hi, existing[1])
                self.letter_hints[letter] = (lo, hi)
            logger.info('Frequency solver seeded %d letters using elimination heuristic', len(self.frequency_solutions))
        logger.info('Constraint analysis complete. Hints=%d Constraints=%d', len(self.letter_hints), len(self.letter_constraints))

    def frequency_guided_solver(self, letter_counts):
        """Approximate letter values by eliminating rare letters first (linearized heuristic)."""
        if self.letter_count_matrix is None:
            return {}
        usage_order = sorted(letter_counts.items(), key=lambda kv: (kv[1], kv[0]))
        solutions = {}
        matrix = self.letter_count_matrix
        use_numpy = np is not None and hasattr(matrix, 'shape')

        def matrix_value(row_idx, col_idx):
            if use_numpy:
                return float(matrix[row_idx, col_idx])  # type: ignore[index]
            return matrix[row_idx][col_idx]
        for letter, _ in usage_order:
            idx = self.letters.index(letter)
            estimates = []
            for row_idx, number in enumerate(self.numbers):
                count = matrix_value(row_idx, idx)
                if count <= 0:
                    continue
                residual = number
                for solved_letter, solved_value in solutions.items():
                    solved_idx = self.letters.index(solved_letter)
                    solved_count = matrix_value(row_idx, solved_idx)
                    if solved_count:
                        residual -= solved_value * solved_count
                approx = residual / count
                if math.isfinite(approx) and abs(approx) < 1e6:
                    estimates.append(approx)
            if estimates:
                value = sum(estimates) / len(estimates)
                solutions[letter] = value
        return solutions
    ...
    def initialize_population(self):
        ...
        freq_defaults = self.frequency_solutions or {}
        ...
        for seed_idx in range(seeded_count):
            ...
            for i in range(26):
                ...
                if letter in self.letter_constraints:
                    # Use fixed constraint value (no noise for exact fixes)
                    value = self.letter_constraints[letter]
                elif letter in freq_defaults:
                    base_val = freq_defaults[letter]
                    jitter = max(0.05, abs(base_val) * 0.2)
                    value = random.uniform(base_val - jitter, base_val + jitter)
        ...
        for _ in range(self.population_size - seeded_count):
            ...
            for i in range(26):
                ...
                if self.least_squares_solution and letter in self.least_squares_solution:
                    ...
                    base = freq_defaults[letter]
                    span = max(0.1, abs(base) * 0.25)
                    value = random.uniform(base - span, base + span)
                else:
        ...
        # Ensure at least one deterministic heuristic individual exists
        if freq_defaults:
            deterministic = {}
            for letter in self.letters:
                if letter in freq_defaults:
                    deterministic[letter] = freq_defaults[letter]
                elif self.least_squares_solution and letter in self.least_squares_solution:
                    deterministic[letter] = self.least_squares_solution[letter]
                else:
                    deterministic[letter] = 1.0
                if self.decimal_places is not None:
                    deterministic[letter] = round(deterministic[letter], self.decimal_places)
            self.population[0] = deterministic
            try:
                self.best_solution = deterministic.copy()
                self.best_fitness, _, _ = self.calculate_fitness(self.best_solution)
            except Exception:
                pass
    ...
    def mutate(self, individual):
        ...
        rare_focus = self.rare_letters if getattr(self, 'rare_letters', None) else []
        for letter in mutated:
            ...
            if random.random() < effective_rate:
                ...
                if mutation_type < 0.5:
                    mutated[letter] += random.uniform(-0.5, 0.5) * scale
        ...
        if rare_focus and random.random() < 0.25:
            target_letter = random.choice(rare_focus)
            span = 1.5 if self.allow_negative_values else 1.0
            mutated[target_letter] += random.uniform(-span, span)
            if self.decimal_places is not None:
                mutated[target_letter] = round(mutated[target_letter], self.decimal_places)
    ...
    def _eval_ind(self, individual):
        ...
        return (fitness, solved, max_err, individual)
    ...
    def evolve_generation(self):
        ...
        with mp.Pool(self.cpu_cores) as pool:
            fitness_scores = pool.map(self._eval_ind, self.population)
    ...
    def optimize(self, max_generations=100, callback=None):
        ...
        attempts = 0
        max_attempts = 2 if self.adaptive_decimals else 1  # Rerun if not perfect
        
        while attempts < max_attempts and self.is_solving:
            attempts += 1
            if so is not None:
                # Use SciPy differential_evolution for better optimization
                def objective(x):
                    letter_values = {self.letters[i]: x[i] for i in range(len(self.letters))}
                    return self.calculate_fitness(letter_values)[0]
                
                bounds = []
                for L in self.letters:
                    if L in self.letter_constraints:
                        v = self.letter_constraints[L]
                        bounds.append((v, v))
                    elif L in self.letter_hints:
                        lo, hi = self.letter_hints[L]
                        bounds.append((lo, hi))
                    else:
                        lo = -10 if self.allow_negative_values else 0
                        hi = 10
                        bounds.append((lo, hi))
                ...
                def de_callback(xk, convergence=None):
                    ...
                    letter_values = {self.letters[i]: xk[i] for i in range(len(self.letters))}
                    fitness, solved, max_err = self.calculate_fitness(letter_values)
                    stats = {
                        'generation': self.generation,
                        'best_fitness': fitness,
                        'solved_count': solved,
                ...
                # Initial population for seeding
                init = 'latinhypercube'  # Better starting points for accuracy
                ...
                res = so.differential_evolution(
                    ...
                    atol=1e-8,
                    polish=True,  # Refine with local optimizer for 100% aim
                    init=init
                )
                self.best_solution = {self.letters[i]: res.x[i] for i in range(len(self.letters))}
                self.best_fitness, solved, _ = self.calculate_fitness(self.best_solution)
                
                # Check if perfect; if not and adaptive, prepare rerun
                if self.best_fitness > 0 and self.adaptive_decimals and attempts < max_attempts:
                    self.decimal_places = min(self.decimal_places + 2, 20)
                    logger.info(f'Rerunning with higher precision: {self.decimal_places} decimals')
                    max_generations = int(max_generations * 1.5)  # Increase iterations
            else:
                # Fallback to original GA
